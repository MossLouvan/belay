...
// own BELAY_MAC_CTRL remap, and two remaps deciding the same keystroke is
...
test('windows driving mac, verbatim: literal Control, not the host remap', () => {
  ...
  // whose BELAY_MAC_CTRL default would rewrite `ctrl` into Command.
...
test('a mac host is only ever spoken to in unambiguous spellings', () => {
  for (const clientIsMac of [true, false]) {
    for (const mode of ['remap', 'verbatim']) {
      ...
      assert.ok(!roles(map).includes('ctrl'), 'ctrl is the name BELAY_MAC_CTRL rewrites');
