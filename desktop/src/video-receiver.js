...
export async function reserveVideo(root, host, deliver, ended) {
  ...
  let pending=Buffer.alloc(0), waiting=false, needKey=true, stopped=false, configured=false;
  ...
  child.stdout.on('data',chunk=>{
    ...
    while(pending.length>=5) {
      ...
      if(waiting) {if(!needKey)child.stdin.write('keyframe\n');needKey=true;continue;}
      if(needKey&&!keyframe) continue;
      needKey=false;waiting=true;deliver({keyframe,data:Uint8Array.from(data)});
    }
  });
  ...
  return {port,stop,ack(){waiting=false;},configure(offer,preset){
