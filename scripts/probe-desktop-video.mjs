...
try {
 receiver=await reserveVideo(resolve(root,'desktop'),'http://127.0.0.1',frame=>{
   frames++;keys+=Number(frame.keyframe);bytes+=frame.data.length;receiver.ack();
 },()=>{});
 ...
 const ready=new Promise((yes,no)=>{
  ...
  lines.on('line',line=>{const m=JSON.parse(line);if(m.type==='error')no(new Error(m.error));if(m.type==='ready'){path=m.path;receiver.configure({port:m.port,key:token,salt},'max');yes();}});
 });
 streamer.stdin.write(JSON.stringify({peer:`127.0.0.1:${receiver.port}`,token,salt,preset:'max',fps:60,source:'synthetic'})+'\n');
 await ready;
 await new Promise(r=>setTimeout(r,8000));
 console.log(JSON.stringify({path,frames,keyframes:keys,bytes,seconds:8}));
