...
const encoder=process.argv.includes('--nvenc')?'nvenc':'mf';
...
try {
 ...
 streamer.stdin.write(JSON.stringify({peer:`127.0.0.1:${proxy?.address().port??receiver.port}`,token,salt,preset:'max',fps:60,fec,source,encoder})+'\n');
 await ready;
 await new Promise(r=>setTimeout(r,durationMs));
 gaps.sort((a,b)=>a-b);
 timerLateness.sort((a,b)=>a-b);
 console.log(JSON.stringify({path,source,encoder,frames,keyframes:keys,bytes,seconds:durationMs/1000,stages,
