# Tether Design System — "Ledger"

Specification for the visual rebuild of the Tether app (Expo / React Native, iOS + web).
...
3. **Mono is the voice of the machine, sans is the voice of the app.** Everything that
   ...
   output, IPs — is set in mono. Everything Tether itself says — titles, buttons, prose,
...
7. **Density with rhythm.** Tether is a utility; it is allowed to be dense. Density is
...
● RUNNING          TETHER          (row: status label in status colour, project
...
|---|---|---|
| Connect | "TETHER" brand block, then the address input | `display` 40/900; the input is the only `surface` fill on the page |
...
Minimal editorial design earns its calm by removing affordances; Tether is a dense
