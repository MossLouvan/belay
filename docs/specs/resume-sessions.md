# Spec: Resume any Claude Code session from the Belay Agent tab

**Status:** ready to implement · **Author:** spec written 2026-08-23 · **Repo:** `<repo>`

## Goal

When the user comes back to this computer (physically or via the Belay phone
app), they should see **every Claude Code session that exists on this machine**
— not just the ones started from Belay — and be able to resume any of them
with one tap, with Belay's phone-approval flow attached.

Today the Agent tab only lists sessions Belay itself created
(`server/belay-agent.json`). Sessions started from a terminal (`claude` in any
project) are invisible to it, even though Claude Code persists all of them.

## Background you need (read first)

- **How Belay agent sessions work now:** `server/src/agent.ts`. Each Belay
...
- **Permission flow (do not break):** every tool use routes through
  `--permission-prompt-tool mcp__belay-approve__request_permission` served by
...
- **Where Claude Code stores sessions on disk:**
  ...
  `C--Users-you-Documents-belay`). **Do not hand-decode the directory name
...
- Exclude sessions already attached to a Belay session
  (`claudeSessionId` match against `belay-agent.json` entries).
...
- `POST /agent/attach` (auth) body
  `{ claudeSessionId, cwd, title? }` → creates a Belay session whose
...
it concurrently from Belay would fork/contend. Best-effort guard:
...
- `--resume` on a session that another process has open must not corrupt
  Belay state: if the spawned process exits immediately with an error, the
...
- Below Belay's own sessions, a section **"On this PC"** listing discovered
...
- **No live end-to-end testing on this machine.** Do not start the Belay
