...
// DESKHANDLER_TEST_PORT overrides it for the case where 8799 is itself taken.

export const HOST_PORT = Number(process.env.DESKHANDLER_TEST_PORT || process.env.TETHER_TEST_PORT) || 8799;
export const APP_PORT = Number(process.env.DESKHANDLER_TEST_APP_PORT || process.env.TETHER_TEST_APP_PORT) || 8081;

export const HOST = `127.0.0.1:${HOST_PORT}`;
export const APP_ORIGIN = `http://127.0.0.1:${APP_PORT}`;

// Matches DESKHANDLER_TEST_CODE, which playwright.config.ts passes to the agent.
