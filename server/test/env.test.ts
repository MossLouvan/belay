// The rename shim: DESKHANDLER_* is canonical, TETHER_* still read.
//
...
test('the canonical DESKHANDLER_* name is read', () => {
  assert.equal(productEnv('PORT', { DESKHANDLER_PORT: '9000' }), '9000');
});
...
test('when both are set, the canonical name wins', () => {
  assert.equal(
    productEnv('PORT', { DESKHANDLER_PORT: '9001', TETHER_PORT: '9000' }),
    '9001',
  );
});

test('an empty canonical value still wins — set is set', () => {
  // Someone exporting DESKHANDLER_HOSTS="" means "no extra hosts", not
  // "please resurrect whatever TETHER_HOSTS said years ago".
  assert.equal(productEnv('HOSTS', { DESKHANDLER_HOSTS: '', TETHER_HOSTS: 'x' }), '');
});
