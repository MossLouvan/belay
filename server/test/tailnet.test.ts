...
test('TETHER_TAILNET_PAIR=0 switches the feature off', async () => {
  const prev = process.env.TETHER_TAILNET_PAIR;
  process.env.TETHER_TAILNET_PAIR = '0';
  try {
    assert.equal(tailnetPairingEnabled(), false);
    assert.deepEqual(await tailnetTrusted('100.101.102.103'), { trusted: false });
  } finally {
    if (prev === undefined) delete process.env.TETHER_TAILNET_PAIR; else process.env.TETHER_TAILNET_PAIR = prev;
  }
});
