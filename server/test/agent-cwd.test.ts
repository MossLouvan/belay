...
before(async () => {
  outside = await mkdtemp(join(tmpdir(), 'tether-agent-cwd-outside-'));
  inside = join(HOME, '.tether-test-agent-cwd');
...
test('~ expands to the host home', () => {
  const resolved = resolveSessionCwd('~/.tether-test-agent-cwd/project');
