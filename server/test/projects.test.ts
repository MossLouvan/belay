...
before(async () => {
  outside = await mkdtemp(join(tmpdir(), 'tether-projects-outside-'));
  inside = join(HOME, '.tether-test-projects');
...
test('the Tether install directory is refused as a parent', async () => {
