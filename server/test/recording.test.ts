...
before(async () => {
  project = join(homedir(), '.tether-test-recordings');
  await rm(project, { recursive: true, force: true });
  await mkdir(project, { recursive: true });
  outside = await mkdtemp(join(tmpdir(), 'tether-rec-outside-'));
});
...
test('discard resets everything and writes nothing to disk', async () => {
  ...
  assert.equal(existsSync(join(project, '.tether')), false);
});
...
test('deliver writes ≤ maxKept ordered frames into the project and resets', async () => {
  ...
  assert.match(delivery.relDir, /^\.tether[\\/]recordings[\\/]rec-\d{8}-\d{6}$/);
...
test('pruneRecordings keeps the newest maxSaved and ignores foreign files', async () => {
  const recordingsDir = join(project, '.tether', 'recordings');
