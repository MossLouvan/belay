// Product-prefixed environment variables, post-rename (Tether → Belay).
// The Swift twin of server/src/env.ts, for the helper's own knobs.

import Foundation

/// A product env var: BELAY_* is canonical, TETHER_* still honoured.
...
func productEnv(_ suffix: String) -> String? {
    ...
    return env["BELAY_" + suffix] ?? env["TETHER_" + suffix]
}
