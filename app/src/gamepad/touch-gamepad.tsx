...
function TouchControl({ rect, state, labels }: { readonly rect: ControlRect; readonly state: SharedValue<GamepadState>; readonly labels: ControllerLabels }) {
  ...
  return <GestureDetector gesture={gesture}>
    <Animated.View testID={`gamepad-${rect.id}`} accessible accessibilityRole={stick ? 'adjustable' : 'button'} accessibilityLabel={label}
      ...
      <View style={stick ? { position: 'absolute', top: 0, width: '100%', minHeight: theme.layout.minTouch, justifyContent: 'center' } : undefined} pointerEvents="none" accessibilityElementsHidden importantForAccessibility="no-hide-descendants">
        <Txt variant="label" align="center" numberOfLines={1}>{label}</Txt>
      </View>
