...
export function useGamepad(enabled: boolean, preset: PresetId, connectionKey: string) {
  ...
  useEffect(() => {
    ...
    const events = gamepadNative?.addListener('onState', raw => { if (live && validState(raw)) controller.current = raw; });
    const connections = gamepadNative?.addListener('onConnection', connection);
    void gamepadNative?.start().then(connection).catch(() => { if (live) setBackend('Physical controller module could not start'); });
    return () => {
  ...
  return { physical, backend, keymap, updateTouch, foreground };
}
