...
export function useGaming(active: boolean, connectionKey: string) {
  ...
  const gamepad = useGamepad(active && enabled, preset, connectionKey);
  useEffect(() => {
    let live = true;
  ...
  useEffect(() => { if (loaded) void AsyncStorage.setItem('belay.gamepad.preferences', JSON.stringify({ preset, layout })).catch(() => { }); }, [loaded, preset, layout]);
  ...
  return {
    enabled, sheet, preset, layout, loaded, ...gamepad, setSheet, setPreset, setLayout,
...
export function GamingSheet({ gaming }: { readonly gaming: GamingState }) {
  return <Sheet title="Gaming" visible={gaming.sheet} onClose={() => gaming.setSheet(false)} testID="gaming-sheet">
    <Column gap="md">
...
export function GamingOverlay({ gaming, width, height, fps, pingMs }: { readonly gaming: GamingState; readonly width: number; readonly height: number; readonly fps: number; readonly pingMs: number | null }) {
  ...
  return <View pointerEvents="box-none" style={{ position: 'absolute', top: insets.top, left: insets.left, right: insets.right, bottom: insets.bottom, zIndex: 8 }}>
    {!gaming.physical && gaming.foreground ? <React.Suspense fallback={null}><TouchGamepad key={`${gaming.layout}-${touchPreset}-${w}-${h}`} width={w} height={h} layout={gaming.layout} preset={touchPreset} onState={gaming.updateTouch} /></React.Suspense> : null}
