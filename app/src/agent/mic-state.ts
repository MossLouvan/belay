...
export function voiceErrorMessage(code: string, detail?: string): string | null {
  switch (code) {
    case 'aborted':
      return null;
    case 'not-allowed':
      return 'microphone or speech recognition access is off for Belay — allow both in Settings';
...
export function permissionProblem(
  ...
  if (!mic?.granted) return 'microphone access is off for Belay — allow it in Settings to talk';
  if (!speech?.granted) return 'speech recognition is off for Belay — allow it in Settings to talk';
  return null;
}
