...
function mixedContentHint(url: string): string {
  ...
  return ' This page is served over HTTPS, so your browser blocks plain-HTTP connections to your PC — open Tether over http:// instead.';
}
...
export function diagnosePairFailure(url: string, raw: string): Diagnosis {
  ...
  if (/invalid or expired/i.test(detail)) {
    return {
      ...
      message: 'Pairing codes are single-use and expire five minutes after they appear. Check the Tether window on your PC for the current code — it refreshes itself — and enter that one.',
    };
  }
